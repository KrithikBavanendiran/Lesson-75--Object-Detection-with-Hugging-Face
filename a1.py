...
def ask_image():
    print("\n Pick an image (JPG/PNG/BMP/WebP/TIFF, max 8MB) from this folder.")
    while True:
        p= input("Image path: ").strip().strip('"').strip("'")
        if not p or not os.path.isfile(p): print("File not found. Try again."); continue
        if os.path.splitext(p)[1].lower() not in ALLOWED: print("Unsupported file type. Try again."); continue
        if os.path.getsize(p)/(1024*1024) > MAX_MB: print("File too large. Try again."); continue
        try: Image.open(p).verify()
        except: print("Invalid image file. Try again."); continue
        return p
...
def infer(path, img_bytes, tries=8):
    ...
    for _ in range(tries):
        ...
        if r.status_code == 200:
            ...
            if not isinstance(d, list): raise RuntimeError("Bad API response format.")
            return d
...
def main():
    ...
    except Exception as e: return print("Error",e)
    ...
    if counts:
        ...
        print("\n No objects detected with confidence > 50%.")
    print("\n Disclaimer: This is an AI model demo. "
          "Detections may be inaccurate or biased."
          "Use it for fun and learning, not for critical decisions.")
